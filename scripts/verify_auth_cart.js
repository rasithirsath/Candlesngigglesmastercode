...
const API_URL = "https://gilded-glow-experience-main.onrender.com/api";
...
async function verify() {
  ...
  if (cartData2.cart.length !== 1 || cartData2.cart[0].id !== "1") {
    console.error(
      "❌ Cart persistence failed. Expected 1 item, got:",
      cartData2.cart,
    );
    process.exit(1);
  }
  ...
  if (cartData3.cart.length !== 1 || cartData3.cart[0].id !== "1") {
    console.error(
      "❌ Cart lost after re-login. Expected 1 item, got:",
      cartData3.cart,
    );
    process.exit(1);
  }
